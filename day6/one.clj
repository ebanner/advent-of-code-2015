...
(defn toggle [grid [i1 j1] [i2 j2]]
  (reduce* [grid grid
    ...
    (update-in grid [i j] #(bit-xor % 1))))

(defn do-instruction [[action [[i j] [ii jj]]] grid]
  (cond
    (= action :TURN-ON) (turn-on grid [i j] [ii jj])
    (= action :TURN-OFF) (turn-off grid [i j] [ii jj])
    (= action :TOGGLE) (toggle grid [i j] [ii jj])))
